...
const RICE_TOPPING_PROTEINS=new Set(["chicken","chicken_thigh","chicken_breast","chicken_drumstick",
  ...
  "grouper","cod","mackerel","tuna","sardine","fish_slices","prawn","squid","clams","oyster","scallops","mussels","dried_shrimp","fish_cake","century_egg","egg"]);
...
const BROTH_BASE={
  ...
  fish_slices:    {body:1, sweetness:1, depth:1, clean:3, marine:2, note:"poaches in minutes \u2014 sliced fish soup, in at the end"},
...
const SEASONING_DIRS=[
  ...
  {key:"bkt_hokkien", label:"Bak kut teh \u2014 Hokkien", region:"Hokkien/Klang",
   markers:["dang_gui","star_anise","cinnamon","cloves_spice","wolfberry","red_dates","dark_soy_sauce"],
   need:1, needsLiquid:true, bakeNote:"the herbs infuse into a simmering broth", note:"dark and herbal \u2014 medicinal warmth"},
  {key:"bkt_pack", label:"Bak kut teh \u2014 from a packet", region:"Singaporean",
   markers:["bkt_pack"], need:1, needsLiquid:true, bakeNote:"the broth IS the dish \u2014 there is nothing to bake",
   note:"the sachet carries the herbs and pepper \u2014 how most kitchens actually make it"},
  ...
  {key:"rempah",      label:"Rempah (Malay/Peranakan)", region:"Malay/Peranakan",
   markers:["rendang_paste","belacan","candlenut","laksa_paste","sambal","laksa_leaf","lemongrass","galangal","kaffir_lime"], need:1, needsLiquid:false, bakeNote:"fry the paste, then rub it on and roast — as in sambal grilled fish",
...
const SEASON_TIMING={
  ...
  bkt_pack:    {when:"start",  mins:0,  label:"Drop in the spice sachet",
                why:"in with the water from the start \u2014 it needs the whole simmer to give up its herbs; lift the sachet out before serving"},
...
function potNameFor(ingIds, R, leadId, opts){
  ...
  if(dir){
    ...
    if(K==="bkt_teochew"||K==="bkt_hokkien"||K==="bkt_pack"){
      const isPork=/pork/.test((R.anc[leadId]&&[...R.anc[leadId]].join(","))||leadId||"");
...
function steamCandidates(ingIds, R, dishes, AFF, opts){
  ...
  PRIORITY.forEach(form=>{
    ...
    const otherDrops=ids.filter(x=>!inDish.has(x)
      ...
      && !steamPairsFn(x))
      .map(id=>{
        const noForm = ((R.byId[id]||{}).category)==="proteins" && steamFormOf(id,R)===null;
        return {id, why: noForm
          ? "set aside \u2014 not suited to steaming; sear or braise it instead"
          : "set aside \u2014 steam it as its own dish, one plate per subject"};
      })
      .concat(vegAside);
  ...
  // a protein with no steaming form (beef, mutton, steak) would otherwise vanish silently — surface
  // it as set aside on the leading card with the real reason.
  if(out.length){
    const shown=new Set(out.flatMap(c=>c.contents||[]));
    const unsteamable=real.filter(id=>((R.byId[id]||{}).category)==="proteins"
      && steamFormOf(id,R)===null && !shown.has(id));
    if(unsteamable.length){
      const already=new Set((out[0].leftOut||[]).map(x=>x.id||x));
      const add=unsteamable.filter(id=>!already.has(id))
        .map(id=>({id, why:"set aside \u2014 not suited to steaming; sear or braise it instead"}));
      out[0].leftOut=(out[0].leftOut||[]).concat(add);
    }
  }
  return out;
}
...
const RICE_DIRS=[
  ...
  {key:"congee", label:"Congee / porridge", region:"Cantonese", mode:"congee",
   markers:["century_egg","dried_scallop","chicken_whole"], need:0,
...
/* how does an ingredient JOIN a starch dish? The judgement is the same whether the
   starch is rice or noodles — prawns want a few gentle minutes, leafy greens go in at
   the very end, tough beef cannot simply be dumped in. What changes is the wording and,
   for fried noodles, the ORDER: there is no base to go into, so "base_in" means cook it
   before the noodles hit the wok.
   starch: "rice" | "fried" | "soup"                                                    */
function alongNote(stage, starch, riceNote){
  if(starch==="rice") return riceNote;
  const N={
    base_in:{fried:"fry it first, before the noodles go into the wok",
             soup:"simmer it in the broth until cooked through"},
    lay_on:{fried:"add it near the end of the toss, once the noodles are coated",
            soup:"poach it in the broth for the last few minutes, or arrange it on the bowl"},
    fold_end:{fried:"in for the last ten seconds only \u2014 it should still have snap",
              soup:"blanch it briefly and lay it on top, so it stays bright"},
    no_along:{fried:"cook it separately and add it at the end",
              soup:"cook it separately \u2014 it will not come good in the broth"}
  };
  return (N[stage]||{})[starch] || riceNote;
}
function starchAlong(id, R, starch){
  starch=starch||"rice";
  ...
  const out=(stage, riceNote, extra)=>Object.assign({stage, note:alongNote(stage,starch,riceNote)}, extra||{});
  if(id==="egg") return out("lay_on","crack it on top for the last few minutes so it steams into the rice, or stir it through the hot rice at the end");
  if(id==="lap_cheong") return out("base_in","slice it in from the start \u2014 the fat renders into the rice");
  if(id==="char_siu") return out("lay_on","already cooked \u2014 lay it on to warm through for the last few minutes");
  if(id==="dried_shrimp") return out("base_in","in from the start \u2014 it perfumes the whole pot");
  if(FISH.includes(id)) return out("lay_on","lay the fillet on the rice for the last 10 minutes \u2014 it steams through gently");
  if(id==="fish_slices") return out("lay_on","slip the slices in for the last few minutes \u2014 they cook in moments and break if stirred");
  if(["prawn","squid","clams","oyster","cockles","scallops","mussels"].includes(id)) return out("lay_on","scatter on top for the last 6\u20138 minutes, just until opaque");
  if(id==="beef_slices") return out("lay_on","thin slices laid on for the last few minutes");
  if(p.has("collagen_rich") || i.cut_role==="braise") return out("no_along","needs 45+ minutes of braising \u2014 far longer than the rice takes",{alt:"a slow braise"});
  if(["beef","beef_steak","beef_chuck","pork_loin","pork_shoulder","mutton"].includes(id)) return out("no_along","wants dry, high heat \u2014 not gentle steam",{alt:"a hot sear"});
  if(/^chicken/.test(id)) return out("base_in","marinate it and cook it on the rice from the start");
  if(id==="tofu"||id==="tau_kee"||id==="tau_pok") return out("base_in","firm tofu holds up cooked in the rice");
  if(id==="fish_cake") return out("lay_on","already cooked \u2014 in for the last few minutes to warm through");
  if(i.category==="vegetables"){
    if(["peas","beansprouts","spinach","bok_choy","chye_sim","kangkong","kailan","watercress","lettuce"].includes(id))
      return out("fold_end","fold through at the very end \u2014 residual heat is enough, and it stays bright");
    return out("base_in","cooks in the same time and heat \u2014 stir it in with the rice");
  }
...
function riceCandidates(ingIds, R, dishes, AFF, opts){
  ...
  // a plain solid protein with no strong style: SURFACE THE FORK — offer a bowl led by EACH one
  // the cook owns (chicken AND prawn), so a second protein isn't only ever set aside. "in the rice"
  // only if it can cook there. Egg and flavour-bits ride along, not lead.
  styledSubjects=new Set(out.map(o=>o.subject));
  const RIDE_ALONG=["dried_shrimp","lap_cheong","egg","century_egg"];
  const meats=proteins.filter(p=>!styledSubjects.has(p) && !fishes.includes(p) && !RIDE_ALONG.includes(p));
  meats.forEach(p=>{
    const sp=(typeof potSpeciesWord==="function") ? potSpeciesWord(p, R) : (nm(p)||p);
    const spCap=sp.replace(/^./,ch=>ch.toUpperCase());
    if(riceAlong(p,R).stage!=="no_along")
      add("in_"+p, spCap+" claypot rice","in_rice", p,"cooked in the rice \u2014 marinate it, cook it on the grain, form a crust");
    add("over_"+p, spCap+" over rice","over_rice", p,"rice separate \u2014 cook it in a sauce, or sear it, and serve over");
  });
  ...
  // seafood leads a claypot bowl too — prawn/squid/scallops laid on the rice as it finishes.
  // (These were excluded from the "plain meat" path above because they cook differently, but
  // they are absolutely a rice subject: prawn claypot rice, etc.)
  if(!out.some(o=>o.mode==="in_rice")){
    const seafood=proteins.find(p=>["prawn","squid","clams","scallops","oyster","mussels"].includes(p));
    if(seafood) add("seafood_"+seafood, nm(seafood)+" claypot rice","in_rice", seafood,
      "cooked in the rice \u2014 the seafood laid on top and steamed through as the grain cooks");
  }
  if(!out.length){
...
/* ============================================================
   NOODLES — the fifth composer.

   A noodle dish is built FROM the noodle: it is the base, not an add-in.
   Two modes for now:
     fried — the noodle goes into the wok last and the sauce coats it
     soup  — the noodle is blanched on its own and broth is poured over

   What makes noodles different from rice: the noodle TYPE constrains the
   dish. Char kway teow needs a flat rice noodle; laksa needs thick bee hoon;
   mee goreng needs yellow noodle. Each direction lists the noodles it works
   with, so the engine never offers a dish the noodle in hand cannot make.
   A direction with noodles:null works with any noodle.
   ============================================================ */
const NOODLE_BASE_IDS=new Set(["bee_hoon","yellow_noodle","mee_pok","hor_fun",
  "udon","soba","noodles","glass_noodles","shirataki"]);

const NOODLE_MODES={
  fried:{key:"fried", label:"Fried in the wok", tag:"high heat \u00b7 sauce coats \u00b7 wok hei",
    why:"everything else is cooked first; the noodle goes in last and takes up the sauce over high heat, so it never stews"},
  soup:{key:"soup", label:"In broth", tag:"noodle blanched \u00b7 broth poured over",
    why:"the noodle is blanched on its own and the broth goes over at the end \u2014 cook it in the soup and it turns soft and clouds the broth"}
};

const NOODLE_DIRS=[
  /* ---- FRIED ---- */
  {key:"mee_goreng", label:"Mee goreng", region:"Malay/Indian-Muslim", mode:"fried",
   canonical:["yellow_noodle"], works:["noodles","bee_hoon","hor_fun"], markers:["ketchup","tau_pok","tomato","sambal"], need:1, defining:["ketchup"],
   ...
   steps:[
     ...
     {t:"Egg first", d:"Scramble the egg in the hot wok and push it aside before the noodles go in."},
     {t:"Fry hard", d:"Toss the noodles through the sauce over the highest heat until the edges catch and colour."},
  ...
  {key:"char_kway_teow", label:"Char kway teow", region:"Teochew/Hokkien", mode:"fried",
   ...
   steps:[
     ...
     {t:"Lard and aromatics", d:"Fry garlic (and lap cheong if you have it) until fragrant, then push aside."},
     {t:"Noodle, then egg", d:"Sear the kway teow in dark and light soy without stirring too much, then crack the egg against the wok and fold it through."},
  ...
  {key:"hokkien_mee", label:"Hokkien prawn mee", region:"Hokkien", mode:"fried",
   canonical:["yellow_noodle","bee_hoon"], works:["hor_fun","mee_pok"], markers:["prawn","squid","lime","pork_belly"], need:1,
   ...
   steps:[
     ...
     {t:"Both noodles", d:"Use yellow noodle and bee hoon together \u2014 the mix is the point."},
     {t:"Braise, don't fry", d:"Add the stock and let the noodles drink it up over medium heat until barely wet."},
  ...
  {key:"bee_hoon_goreng", label:"Fried bee hoon", neutralLabel:"Simple fried noodles", region:"Singaporean", mode:"fried",
   ...
   steps:[
     ...
     {t:"Season light", d:"Light soy, white pepper and a splash of stock; dark soy only for colour."},
  ...
  {key:"dark_soy_fried", label:"Dark soy fried noodles", region:"Cantonese", mode:"fried",
   ...
   steps:[
     ...
     {t:"High heat, little sauce", d:"Too much liquid steams the noodle instead of frying it."},
     {t:"Finish", d:"Scallion and a few drops of sesame oil off the heat."}]},

  /* ---- SOUP ---- */
  {key:"laksa", label:"Laksa", region:"Peranakan", mode:"soup",
   canonical:["bee_hoon"], works:["hor_fun","yellow_noodle","mee_pok"], markers:["laksa_paste","coconut_milk","laksa_leaf"], need:1, defining:["laksa_paste"],
   ...
   steps:[
     ...
     {t:"Coconut last", d:"Add stock first, then the coconut milk near the end, and never let it boil hard or it will split."},
  ...
  {key:"fishball_soup", label:"Fishball noodle soup", region:"Teochew", mode:"soup",
   ...
   steps:[
     ...
     {t:"Blanch the noodle", d:"Ten to fifteen seconds in boiling water, then shake it dry \u2014 mee pok goes soft fast."},
     {t:"Assemble", d:"Noodles in the bowl, fish cake and greens on top, then ladle the broth over."},
  ...
  {key:"tom_yum", label:"Tom yum noodles", region:"Thai", mode:"soup",
   ...
   steps:[
     ...
     {t:"Lime off the heat", d:"Lime juice goes in once the pot is off the flame \u2014 boiled lime turns bitter."},
     {t:"Assemble", d:"Blanched noodles in the bowl, broth over, coriander on top."}]},
  {key:"fish_soup", label:"Sliced fish bee hoon", region:"Teochew/Singaporean", mode:"soup",
   ...
   steps:[
     ...
     {t:"The broth", d:"Pork or anchovy stock, brought to a gentle simmer. A splash of evaporated milk turns it the milky white version."},
     {t:"Fish in last", d:"Slip the slices in off the boil for two minutes only \u2014 they cook in moments and break if stirred."},
  ...
  {key:"sliced_meat", label:"Sliced meat noodle soup", region:"Chinese", mode:"soup",
   ...
   steps:[
     ...
     {t:"Keep the broth clear", d:"A gentle simmer only. Skim what rises."},
     {t:"Meat at the last moment", d:"Slide the slices in off the boil and take the pot off the heat \u2014 they finish in the residual heat."},
  ...
  {key:"dashi_noodle", label:"Dashi noodle soup", region:"Japanese", mode:"soup",
   ...
   steps:[
     ...
     {t:"Season lightly", d:"Shoyu and mirin only. The broth should stay clear enough to see the bottom of the bowl."},
     {t:"Noodles separately", d:"Cook and rinse the noodles in cold water to stop them, then warm them through before the broth goes over."},
  ...
  {key:"clear_soup", label:"Clear soup noodles", region:"Chinese", mode:"soup",
   ...
   steps:[
     ...
     {t:"Blanch separately", d:"Never cook the noodles in the broth, or the starch clouds it."},
     {t:"Assemble", d:"Noodles in the bowl, toppings on, broth over at the last moment."}]}
];

/* which noodle is the base? the first one present, by a sensible preference */
function noodleBaseOf(ids){
  const order=["bee_hoon","yellow_noodle","mee_pok","hor_fun","noodles","udon","soba","glass_noodles","shirataki"];
  return order.find(n=>ids.includes(n)) || ids.find(id=>NOODLE_BASE_IDS.has(id)) || null;
}

/* how well does this noodle suit this style?
   canonical = what it's traditionally made with; works = commonly ordered that way;
   unusual = you can, and the engine will let you, but it's not what the dish normally is.
   Nothing is forbidden — the stall will make laksa with hor fun if you ask. */
function noodleFitOf(dir, base){
  if(!dir.canonical) return "canonical";            // style has no fixed noodle
  if((dir.canonical||[]).includes(base)) return "canonical";
  if(!dir.works) return "unusual";
  return (dir.works||[]).includes(base) ? "works" : "unusual";
}
const NOODLE_FIT_RANK={canonical:0, works:1, unusual:2};
...
function noodleCandidates(ingIds, R, dishes, AFF, opts){
  ...
  out.sort((a,b)=> (b.isDefining?1:0)-(a.isDefining?1:0) || b.hits - a.hits || NOODLE_FIT_RANK[a.fit] - NOODLE_FIT_RANK[b.fit]);

  // MULTIPLE PROTEINS: a bowl leads with one, but the cook may want a chicken bowl rather than a
  // prawn bowl. Add a variant of the best style led by each OTHER protein so every protein the
  // cook owns gets a seat, instead of being only ever set aside. (Whole fish excluded.)
  const allProteins=mineAll.filter(id=>((R.byId[id]||{}).category)==="proteins" && !isWholeFish(id,R)
    && id!=="egg");
  if(allProteins.length>1 && out.length){
    const leadPj=pickLeadProtein(allProteins, R);
    const topStyle=out[0];
    const others=allProteins.filter(id=>id!==leadPj.lead);
    others.forEach(protId=>{
      const pName=(R.byId[protId]||{}).name||protId;
      const dir=topStyle.direction;
      const vpick=pickDishVeg(mineAll, R, {greens:1, others:2, prefer:(dir&&dir.veg)||[], explicit:(opts&&opts.explicit)||null, benched:(opts&&opts.benched)||null});
      const contents=vpick.used.filter(id=>{
        if(isWholeFish(id,R)) return false;
        if(((R.byId[id]||{}).category)!=="proteins") return true;
        return id===protId;
      });
      const otherProt=allProteins.filter(id=>id!==protId);
      out.push({
        key:"noodle_"+(dir?dir.key:"clear_soup")+"_"+protId,
        label:(topStyle.fit==="canonical"?topStyle.label:pName+" "+(dir&&dir.neutralLabel?dir.neutralLabel.toLowerCase():"noodles")),
        mode:topStyle.mode, dirKey:dir?dir.key:"clear_soup", direction:dir, isDefining:false,
        subject:base, subjectName:baseName, note:topStyle.note, fit:topStyle.fit, hits:topStyle.hits,
        leadProtein:protId, contents,
        leftOut:vpick.leftOut.concat(otherProt.map(id=>({id,
          why:"one main is enough in a bowl \u2014 this is the "+pName.toLowerCase()+" version"})))
      });
    });
  }

  const noodleWord = /mee|yellow_noodle/.test(base) ? "mee"
                   : /bee_hoon/.test(base) ? "bee hoon"
                   : /hor_fun/.test(base) ? "hor fun"
                   : "noodles";
  out.forEach(c=>{
    if(c.leadProtein){
      const sp=(typeof potSpeciesWord==="function") ? potSpeciesWord(c.leadProtein, R) : ((R.byId[c.leadProtein]||{}).name||c.leadProtein);
      const spCap=sp.replace(/^./,ch=>ch.toUpperCase());
      c.label = c.mode==="soup" ? spCap+" "+noodleWord+" soup" : spCap+" "+noodleWord;
    }
  });
  const variants=out.filter(c=>c.leadProtein);
  const mains=out.filter(c=>!c.leadProtein);
  const ordered=mains.length ? [mains[0]].concat(variants, mains.slice(1)) : variants;
  ...
  if(!ordered.length){
    ...
    ordered.push({key:"noodle_clear_soup", label:"Noodles in broth", mode:"soup",
...
/* THE SECOND LAYER — given a style, which noodles can carry it, best first?
   This is what a "change the noodle" picker reads: every noodle is offered, ordered
   canonical → works → unusual, each with a plain-language note. Nothing is hidden,
   because a stall will swap the noodle for you if you ask. */
function noodleOptionsFor(dirKey, R, opts){
  opts=opts||{};
  const dir=NOODLE_DIRS.find(d=>d.key===dirKey);
  if(!dir) return [];
  const have=new Set(opts.have||[]);
  const all=["bee_hoon","yellow_noodle","mee_pok","hor_fun","noodles","udon","glass_noodles","soba","shirataki"];
  return all.map(id=>{
    const fit=noodleFitOf(dir, id);
    const name=(R.byId[id]||{}).name||id;
    return {
      id, name, fit,
      owned:have.has(id),
      note: fit==="canonical" ? "what it's usually made with"
          : fit==="works"     ? "a common swap"
          : "unusual, but it will work"
    };
  }).sort((a,b)=> NOODLE_FIT_RANK[a.fit]-NOODLE_FIT_RANK[b.fit] || (b.owned?1:0)-(a.owned?1:0) || a.name.localeCompare(b.name));
}

/* the recipe for one noodle candidate */
function buildNoodle(ingIds, R, dishes, AFF, opts){
  opts=opts||{};
  const real=[...new Set(ingIds.filter(id=>{
    const i=R.byId[id]||{};
    if(i.id==="water") return false;
  ...
  const nm=id=>((R.byId[id]||{}).name)||id;
  const base=opts.subject||noodleBaseOf(real);
  const dir=opts.direction||NOODLE_DIRS.find(d=>d.key===opts.dirKey)||NOODLE_DIRS.find(d=>d.key==="clear_soup");
  const mode=opts.mode||dir.mode;
  const modeInfo=NOODLE_MODES[mode]||NOODLE_MODES.soup;

  const prep=[];
  const steps=[];
  let handsOff=0;

  // the noodle's own prep — this is where noodle dishes go wrong
  if(base==="bee_hoon")
    prep.push({name:nm(base), prep:"soak in warm water until pliable but still firm \u2014 boiled bee hoon breaks apart in the wok", warn:null});
  else if(base==="mee_pok")
    prep.push({name:nm(base), prep:"loosen the strands before blanching so they don't cook in a clump", warn:null});
  else if(base==="yellow_noodle")
    prep.push({name:nm(base), prep:"rinse under running water to wash off the alkaline smell, then drain well", warn:null});
  else if(base==="hor_fun")
    prep.push({name:nm(base), prep:"separate the sheets gently by hand \u2014 they tear if you pull", warn:"cold kway teow breaks; let it come to room temperature first"});
  else if(base)
    prep.push({name:nm(base), prep:"cook just short of done, then drain and rinse", warn:null});

  // the other solids — staged by HOW they join the dish, using the shared starch model
  const starchKind = mode==="fried" ? "fried" : "soup";
  const others=real.filter(id=>id!==base && !NOODLE_BASE_IDS.has(id) &&
    ["proteins","vegetables"].includes((R.byId[id]||{}).category));
  const staged=others.map(id=>Object.assign({id, name:nm(id)}, starchAlong(id, R, starchKind)));
  // rank within each stage by cuisine fit so the core ingredients lead
  const rank=ids=>R.rankPartners?R.rankPartners(ids):ids;
  const byStage=s=>rank(staged.filter(a=>a.stage===s).map(a=>a.id))
                    .map(id=>staged.find(a=>a.id===id));
  const early=byStage("base_in"), late=byStage("lay_on"),
        endFold=byStage("fold_end"), apart=byStage("no_along");
  staged.forEach(a=>{
    const i=R.byId[a.id]||{};
    prep.push({name:a.name, prep:i.prep_verb||"prepare", warn:null});
  });
  const listOf=arr=>arr.length ? arr.map(a=>a.name).join(", ").replace(/, ([^,]*)$/," and $1") : null;
  const otherNames=listOf(staged);

  if(mode==="fried"){
    steps.push({label:"Heat the wok hard", detail:"A fried noodle is made by heat. Get the wok smoking before anything goes in \u2014 a cool wok steams the noodle instead of frying it.", mins:2, handsoff:false});
    steps.push({label:"Aromatics first", detail:"Garlic and the other aromatics into the hot oil until fragrant.", mins:1, handsoff:false});
    if(early.length) steps.push({label:"Cook "+listOf(early)+" through",
      detail:"These need real cooking time, so they go in before the noodles: "+early.map(a=>a.name.toLowerCase()+" \u2014 "+a.note).join("; ")+".", mins:3, handsoff:false});
    steps.push({label:"Noodles in last", detail:"In with the noodles and the sauce. Toss by lifting and separating rather than stirring, so the strands don't clump or tear.", mins:3, handsoff:false});
    if(late.length) steps.push({label:"Add "+listOf(late),
      detail:late.map(a=>a.name.toLowerCase()+" \u2014 "+a.note).join("; ")+".", mins:2, handsoff:false});
    if(endFold.length) steps.push({label:"Greens at the very end",
      detail:listOf(endFold)+" \u2014 "+endFold[0].note+".", mins:1, handsoff:false});
    steps.push({label:"Finish", detail:"Scallion off the heat, and serve straight away \u2014 fried noodles go soft as they sit.", mins:1, handsoff:false});
  } else {
    steps.push({label:"Build the broth", detail: early.length
      ? "Get the broth simmering, then "+early.map(a=>a.name.toLowerCase()+" \u2014 "+a.note).join("; ")+"."
      : "Get the broth simmering and season it.", mins:15, handsoff:true});
    handsOff=15;
    if(late.length) steps.push({label:"Add "+listOf(late),
      detail:late.map(a=>a.name.toLowerCase()+" \u2014 "+a.note).join("; ")+".", mins:3, handsoff:false});
    steps.push({label:"Blanch the noodles separately", detail:"In boiling water for seconds only, then shake dry. Cooking them in the broth clouds it with starch and turns them soft.", mins:2, handsoff:false});
    steps.push({label:"Assemble the bowl", detail:"Noodles in first, toppings arranged on top, then ladle the hot broth over at the last moment.", mins:2, handsoff:false});
    steps.push({label:"Season the bowl", detail:"White pepper, a few drops of sesame oil \u2014 in the bowl, not the pot, so each bowl is seasoned to taste.", mins:1, handsoff:false});
  }

  const prepMins=Math.max(4, 3+prep.length*2);
  const cookMins=steps.reduce((n,s)=>n+(s.mins||0),0);
  const total=prepMins+cookMins;

  let suggest={yours:[],buy:[]};
  if(AFF){
    const key=others.length?others:[base].filter(Boolean);
    if(key.length) suggest=potSuggestions(key, mode==="fried"?"stir_fry":"soup", R, AFF, {have:real});
  }

  return {
    mode, modeInfo, subject:base, subjectName:nm(base),
    separate: apart.map(a=>({name:a.name, alt:a.alt, note:a.note})),
    label:dir.label, dirKey:dir.key, direction:dir,
    prep, prepMins, steps, total, cookMins,
    handsOn:total-handsOff, handsOff,
    sauceSteps: dir.steps||[],
    suggest,
    warnings: mode==="soup"
      ? ["Never boil the noodles in the broth \u2014 the starch clouds it."]
      : ["Serve fried noodles immediately; they soften as they sit."],
    reminds: []
  };
}
