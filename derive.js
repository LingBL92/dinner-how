...
function wokCandidates(ingIds, R, dishes, AFF){
  ...
  Object.entries(bySp).forEach(([sp,ids])=>{
    ...
    // affinity: which of your vegetables have been stir-fried with this protein?
    const scored=veg.map(v=>({id:v, n:AFF?precedentFor(lead,v,"stir_fry",AFF,R).n:0}))
      .sort((a,b)=>b.n-a.n);
    const pick=scored.slice(0,2).map(x=>x.id);      // a wok holds a protein and 1-2 veg
    out.push({
      key:"prot_"+sp,
      label:(R.byId[lead]||{}).name+" stir-fry",
      lead:[lead],
      contents:[lead].concat(pick).concat(rest),
      note:pick.length? "with "+pick.map(v=>(R.byId[v]||{}).name.toLowerCase()).join(" and ")
                      : "just the "+((R.byId[lead]||{}).name||"").toLowerCase(),
