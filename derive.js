...
const BROTH_BASE={
  ...
  beef_steak:     {body:1, sweetness:1, depth:2, clean:1, marine:0, note:"lean \u2014 not really a broth cut"},
  pork_loin:      {body:1, sweetness:2, depth:1, clean:2, marine:0, note:"lean \u2014 a light, clean broth"}
};
...
function potCandidates(ingIds, R, dishes, AFF){
  const real=ingIds.filter(id=>measureTypeOf(R.byId[id])!=="assumed");
  const proteins=real.filter(id=>(R.byId[id]||{}).category==="proteins" && BROTH_BASE[id]);
  const others  =real.filter(id=>!proteins.includes(id));
  ...
  const pots=Object.entries(bySpecies).map(([sp,ids])=>{
    ...
    others.forEach(o=>{
      // physics: will it survive alongside this base? (leafy + a 90-min braise is fine \u2014
      // it goes in at the end. so physics rarely vetoes. affinity is the real test.)
      let seen=0;
      ids.forEach(p=>{
        ["soup","braise"].forEach(ctx=>{
          const pr=AFF?precedentFor(p,o,ctx,AFF):{n:0};
    ...
    const contents=ids.concat(belongs.map(x=>x.id));
